...
class Post extends Controller {
	...
	{	
		...
		$comments=$this->model->getcomments($id);
            ...
            $result['body']=$this->__clickable($result['body']);
...
private function __clickable($data=null){
$data = trim($data);
			
    while ($data != stripslashes($data)) { $data = stripslashes($data); }    
    $data = strip_tags($data,"<b><i><u>");
    $data = preg_replace("/(?<!http:\/\/)www\./","http://www.",$data);
    $data = preg_replace( "/((http|ftp)+(s)?:\/\/[^<>\s]+)/i", "<a href=\"\\0\" target=\"_blank\">\\0</a>",$data);
    return $data;	
